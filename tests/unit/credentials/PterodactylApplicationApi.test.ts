import { PterodactylApplicationApi } from '../../../credentials/PterodactylApplicationApi.credentials';

describe('PterodactylApplicationApi Credentials', () => {
	let credentials: PterodactylApplicationApi;

	beforeEach(() => {
		credentials = new PterodactylApplicationApi();
	});

	describe('credential metadata', () => {
		it('should have correct name', () => {
			expect(credentials.name).toBe('pterodactylApplicationApi');
		});

		it('should have correct display name', () => {
			expect(credentials.displayName).toBe('Pterodactyl Application API');
		});
	...
	describe('properties', () => {
		...
		it('should describe API key format (ptla_ prefix)', () => {
			const apiKeyProp = credentials.properties.find((p) => p.name === 'apiKey');
			expect(apiKeyProp?.description).toContain('ptla_');
		});
	});
	...
	describe('credential test request', () => {
		...
		it('should test against /api/application/users endpoint', () => {
			expect(credentials.test.request.url).toBe('/api/application/users');
		});
